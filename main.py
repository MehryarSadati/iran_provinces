...
language = screen.textinput(
    title="language",
    prompt="would you rather english names or persian names?(english/persian)",
)
if language == "english":
    data = english_data
else:
...
while is_game_on:
    answer = screen.textinput(
        title=f"{right_answers}/31 ostans correct", prompt="name a ostan in iran:"
    ).lower()
    if answer == "exit":
        is_game_on = False
